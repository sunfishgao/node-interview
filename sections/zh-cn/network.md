...
封包/拆包是目前业内常见的解决方案了. 即给每个数据包在发送之前, 于其前/后放一些有特征的数据, 然后收到数据的时候根据特征数据分割出来各个数据包. 一般来说有如下方案:

1. 长度+变长信息 (要正确处理长度信息, 通常要大小端转换)
 
2. JSON、XML (在判断是否收全数据上会比较费劲)
...
在 connect 调用之前,如果没有通过 bind 指定一个端口号, 那么内核会随机分配一个端口(比如 50000). 如果你要连接的目的端口也是本机的 50000 端口, 就会可能出现自己连接到自己的情况. 这个时候你发送数据会发送给自己. Linux下通常需要 getsockname 函数判断一下否是自连接.
...
1. 使用 SO_LINGER 套接字选项，关闭套接字时跳过四次挥手直接发送 RST ，这样就没有 TIME_WAIT 状态。

2. 启用 tcp_tw_recycle 而实现对 TIME_WAIT 快速的回收操作。

3. 设置 tcp_max_tw_buckets 的值限制 TIME_WAIT 数量。

方案 1 可能会造成发送数据不完整。

但 TIME_WAIT 大部分情况是我们的好朋友，没有特别必要可能并不需要限制他。一个友好的设计，通常是由客户端绅士地主动关闭连接，这时候 TIME_WAIT 则是客户端拿去了。服务端再做一个定时清理垃圾链接的操作就可以。维持 TIME_WAIT 连接时所需的开销仅为正常连接的十分之一左右，很多时候并不一定会产生性能问题。


### 半打开

连接的某一方 (可能是服务端也可能是客户端) 由于崩溃或者其他非正常情况断开连接时, 有可能不会执行四次挥手的过程. 这时候另一端还认为连接是连通的. 当对方重启完成, 再向他发数据, 会得到 RST.（两次收到 RST 在 Linux 会得到 SIGPIPE 信号, 收到这个信号的默认行为是关闭进程, 安全起见需要忽略此信号, 但更重要的是避免向已关闭套接字的对端发送两次以上的消息.）


## 半关闭

由于 TCP 是全双工的协议, 当某一方不再需要发送数据, 可以通过 shutdown 函数的第二个参数传递 SHUT_WR 来关闭发送通道, 但是可以继续接受对方的数据.

```c
int shutdown(int sockfd, int howto)
```
合理使用半关闭, 可以让服务端避免进入 TIME_WAIT 状态.
